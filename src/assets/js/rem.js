...
(function (psdWidth, dividendFontSize, maxRootFontSize) {
  ...
  var style = document.createElement('STYLE') // 采用拼css的形式，而不是直接用document.documentElement.style.fontSize的形式的原因是，拼css可以通过加入!important获得最高优先级，http://jsbin.com/dopupudago/2/edit?html,js,output
  ...
  var hasGbcr = 'getBoundingClientRect' in document.documentElement
  var lastRootFontSize = null // 上一次设置的html的font-size
  function setRem () {
    var rootFontSize = Math.min(
      (
        hasGbcr
          ? document.documentElement['getBoundingClientRect']().width // document.documentElement.getBoundingClientRect() iOS4.0+ 安卓2.0+  https://developer.mozilla.org/zh-CN/docs/Web/API/Element.getBoundingClientRect
          : window.innerWidth
    ...
    if (rootFontSize !== lastRootFontSize) {
      // return
      style.innerHTML = 'html{' + 'font-size:' + rootFontSize + 'px!important;' + textSizeAdjustCSSAll + '}'
  ...
  if ('on' + 'pageshow' in window) {
    ...
    window['addEventListener']('load', trySetRem, false)
  }

  window['addEventListener']('resize', trySetRem, false)
